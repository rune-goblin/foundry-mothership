...
#### RC1. preCreateActor writes token fields that do not exist — Silently Failing Write — Critical `[local]`
...
#### RC2. `foundry.appv1` referenced at init — Deprecated API on a removal clock — Critical `[local]`
...
#### RC3. Hotbar macros for gear crash on `item.id` — `id` vs `_id` — Critical `[local]`
...
#### RC4. UUIDs parsed by `split(".")`, guards placed after the dereference — Primitive Obsession — Major `[local]`
...
#### RC5. `rollStatMacro` calls a method that does not exist — Lava Flow on the public API — Major `[local]`
...
#### RC6. The macroTarget dispatch is copy-pasted five times — DRY / Missing Abstraction — Major `[cross-cutting]`
...
#### RC7. The entry module is a grab-bag and imports itself into a cycle — Big Ball of Mud seed — Major `[cross-cutting]`
...
#### RC8. `fromIdUuid` — a God Function scanning every pack per call — Shotgun Search — Major `[local]`
...
#### RC9. `MothershipItem` is a husk with wiring to nowhere — Lava Flow — Minor `[local]`
...
#### RC11. Vocabulary from cut content survives in live dispatch — Lava Flow — Minor `[local]`
...
#### RC12. Debug `console.log` as the only observability; `init*` neither await nor catch — Logging Noise / Swallowed Errors — Minor `[local]`
...
#### RC13. Rolltable ids duplicated from `content/ids.json` — Hidden Coupling — Minor `[cross-cutting]`
...
#### RC14. Settings bypass i18n; `macroTarget`'s scope contradicts its own dialog — Inconsistent Interface — Minor `[local]`
...
#### RC15. Three copy-pasted `diceSoNiceReady` hooks — DRY — Nit `[local]`
...
#### F1. MothershipActor does everything — God Object — Critical `[cross-cutting]`
...
#### F2. rollTable's fallback dialog is hardcoded to Panic and shadows the correct one — Buggy + Dead Code — Major `[local]` ✓ verified
...
#### F4. Stale asset paths in condition-damage chat cards — rename leftover — Major `[local]`
...
#### F5. A standalone damage roll spends ammo — Sequential Coupling / double-spend — Major `[local]` ✓ verified
...
#### F6. Dialog promises that never resolve are awaited — Broken Promise — Major `[cross-cutting]`
...
#### F7. modifyActor is two 140-line copies of itself — Duplicated Code — Major `[local]`
...
#### F8. parseRollResult: 400 lines mixing die-picking with chat HTML — Long Method — Major `[local]`
...
#### F9. rollCheck: 434 lines of branch-per-use-case — Long Method / Divergent Change — Major `[local]`
...
#### F10. Fire-and-forget updates read back stale state — Race Hazard — Major `[cross-cutting]`
...
#### F11. Every consumer re-parses the roll string by substring — Primitive Obsession — Major `[cross-cutting]`
...
#### F12. Dotted field addresses resolved by reduce, updates built by JSON.parse — Stringly Typed — Major `[cross-cutting]`
...
#### F13. Table and flavor identity by munged display names — Stringly Typed dispatch — Major `[cross-cutting]`
...
#### F14. Sentinel values smuggled through unrelated parameters — Control Coupling — Major `[cross-cutting]`
...
#### F15. Anemic Item, all weapon behaviour on the actor — Feature Envy — Major `[cross-cutting]`
...
#### F16. Dead code: unreachable statements, uncalled methods, vestigial branches — Lava Flow — Minor `[local]`
...
#### F18. fromIdUuid's null result is consumed unchecked — Swallowed failure — Minor `[cross-cutting]`
...
#### F19. The enrichment step rewrites Foundry Roll internals — Inappropriate Intimacy — Major `[local]`
...
#### F20. Localization doubles as a data table with a silent fallback — fail-quietly — Minor `[cross-cutting]`
...
#### F21. Rulebook constants embedded inline — Magic Numbers — Minor `[cross-cutting]`
...
#### F22. Creature rolling Panic result 19 crashes on missing class — Null dereference — Minor `[local]`
...
#### F23. chooseSkill builds its list by placeholder string surgery — Poor Man's Templating — Minor `[local]`
...
#### F25. _deriveCharacter and _deriveCreature share three verbatim blocks — Duplicated Code — Minor `[local]`
...
#### F26. Persistent console.log narration throughout the pipeline — Debugging Leftover — Nit `[cross-cutting]`
...
#### U4. Every roll call threads a seven-slot positional API of nulls — Long Parameter List — Major `[cross-cutting]`
...
#### U5. Game rules live inside components — Business Logic in the Presentation Layer — Major `[cross-cutting]`
...
#### U10. Three modules build dialog UI as concatenated HTML strings — Stringly Typed UI — Major `[cross-cutting]`
...
#### U14. XP clamps to 16 on a 15-pip track — Off-By-One — Minor `[local]`
...
#### C1. Death Save macro reads a settings namespace that no longer exists — Shipped bug via Stringly Typed code — Critical `[local]` ✓ verified
...
#### C2. Wound Roll macro embeds bare document ids every integrity guard is blind to — Magic Strings — Major `[local]`
...
#### C3. 1,346 lines of JavaScript the type-checker never sees — Stringly Typed Logic at scale — Major `[cross-cutting]`
...
#### C4. Nothing verifies the committed `packs/_source/` matches the catalogs — Stale Generated Artifact — Major `[local]`
...
#### C5. `npm run test:e2e` rebuilds packs but never propagates them to the harness — Half-Automated Process — Major `[local]`
...
#### C9. The macro cross-product is unrolled by hand — DRY (mechanical data) — Minor `[local]`
...
#### C10. `MACRO_LABEL` restates macro names with a silent fallback — Quiet Degradation — Minor `[local]`
...
#### T1. The macro-facing half of the actor API is tested by nothing — test-coverage — Critical `[cross-cutting]`
...
#### T2. Roll resolution is covered only to the dialog boundary — test-coverage — Major `[local]`
...
#### T8. `_deriveCreature` has no unit test though it is stub-reachable — test-coverage — Minor `[local]`
