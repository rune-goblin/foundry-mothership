...
| **Next** | **Nothing — the ledger is complete (R0–R7 done, 2026-08-13).** The remade system ships; S9 (psg-core.md) holds the remaining UI work. Pushed 2026-08-13. The archive refs were **trimmed by owner decision**: `78841ed` (R4b, the last commit holding legacy) is a plain ancestor of master, so `git show 78841ed:module/actor/actor.js` recovers any legacy file with no ref needed. |
...
| R3-1 | A skill check added the skill's stored `system.bonus` | The skill's **rank** decides via `rankBonus(skillRank)` (PSG 22); the stored bonus is the fallback only for a rank the book does not name | `test/checks.test.ts` |
| R3-2 | Wound links in chat were `@UUID[…Macro…]` strings assembled from a lang-file lookup table | `woundEffect` renders as `@Table[gunshot +]` action text — F13/F20/C2 die together | `test/checks-damage.test.ts` |
| R3-3 | `noCharSelected` told players to change a world-scoped setting only the GM can reach | The dialog says how to select a character or token; the setting-scope decision itself is R4's | — |
| R3-4 | Reload ran from a parked, never-resolving dialog promise; the attack silently did not continue | The prompt resolves, the attack deliberately does not proceed, and the reload card posts | `test/checks.test.ts` |
| R3-5 | — (new semantics) | A bare `@Check[fear]` opens the prompt, where conditions preselect (§34); `@Check[fear -]` states the modifier; `@Table[key]` rolls immediately | `test/checks-actions.test.ts` |
| R4a-1 | Bleeding damage posted a bespoke HTML card with a pre-rename image path | `takeBleedingDamage()` shares `@Gain[health -bleeding]`'s path: the standard mutation card, real asset path | `test/actor-document.test.ts` |
| R4a-2 | `initRollTable(<any id>)` rolled any world/pack RollTable through the global scanner | Only the seven keyed tables roll (a GM's re-pointed table setting is honored); anything else warns `DocumentNotFound`. Hand-scripted arbitrary-table macros have the core alternative `game.tables.get(id).draw()` | `test/api-legacy.test.ts` |
| R4a-3 | `initRollCheck` with an unknown attribute threw a TypeError per token | One `Mosh.Errors.UnknownArgument` warning, nothing rolled | `test/api-legacy.test.ts` |
| R4a-4 | `initModifyActor(addr, 0, null, …)` reached `new Roll(undefined)` and threw | A zero amount is a change of nothing | `test/api-legacy.test.ts` |
| R4a-5 | `modifyItem` on an ability/class fell through every branch and posted an empty card | Any item type is granted, announced with `Mosh.YouAddThisToYourInventory` | `test/actor-document.test.ts` |
| R4a-6 | Hotbar macros stamped flags under the dead `"mosh"` namespace | `flags.mothershiprpg.itemMacro`; the command calls `rollItem` | `test/init.test.ts` |
| R4b-1 | Hotbar macros embedded 40–120-line bespoke dialogs; conditions and the Panic table linked macro documents | All nine hotbar macros are one-line API calls whose prompts are the system's dialogs (R5 grew the amount and wound-table prompts); conditions and Panic rows carry `@Check`/`@Gain`/`@Apply` actions | pipeline guards + `test/content-pipeline.test.ts` |
| R5-1 | `system.json` declared Stress as the token's second bar while the create hook wrote broken bar paths | Both agree: bar 1 Health, bar 2 Wounds. Tokens reading manifest defaults show Wounds where they showed Stress | `test/init.test.ts` + `system.json` |
| R7-1 | Generated characters carried only `system.class.value`; every granted item posted its own chat card | The generator embeds the chosen class item (deleting any prior) and grants silently — the rolls and table draws already narrate | `test/e2e/actor-generator.spec.ts` |
| R7-2 | — (correction) | R3's Wound Roll port silently opened on Bleeding; R7 restores legacy's Blunt Force default. `pickStat` buttons show localized names, not raw keys | `test/ui-dialogs.test.ts` |
...
recoverable from plain history** (the pre-deletion tree is master's own ancestor `78841ed`; the temporary archive refs were trimmed after the swap proved out — the owner's call, and safe because a label on an ancestor preserves nothing history doesn't).
...
The audit's findings resolved here get `[done]` tags
...
| R3 — checks, dialogs | Opus | done | `R3: checks dispatch on a union…` | 602 vitest; 33 mutants, 0 survivors; dist byte-identical. All five owed items landed. AUTOFAIL decided book-scope (`CHECK_SEMANTICS.autoFail`; default `?? true` keeps legacy equivalence — no reachable roll differs, so no divergence row). Divergences R3-1…R3-5 recorded. `check`/`table`/`gain` action slots filled, binding at R5 via `registerCheckActions`; `apply` is R4's. Dialog helper resolves on every path incl. dismissal; mount-once remount caveat noted for R7. |
| R4 — API, content regen | Opus (API) / Sonnet (catalog regen) | done | `R4a: the actor document…` + `R4b: the content calls…` | 770 vitest; packs 111 modified / 0 added / 0 removed, ids byte-identical; macros.ts 1346→490 as generated verb+argument records; three emit-time guards (settings.get / bare id / retired init*) proven to fail on bad fixtures; `CONDITION_IDS` is a leaf `module/conditions.ts` with the one `isCondition` predicate on read AND write (review-blocked once: the id-only match could not survive Foundry's id-stripping — fixed, regression-proven by revert, `keepId` scoped to first copies). **R5 handoff:** flip the 8 `fixme` e2e specs (the panic one must pass advantage or it hangs on the prompt; the dice rig is unverified against the live harness); grow the amount + wound-table prompts so the last four hotbar macros become one-liners (`ChooseAttribute` already carries saves); retarget `test/condition-modifiers.test.ts` and drop the dual-run legacy arms when the files die; align `system.json`'s `secondaryTokenAttribute` with the hook (decide the owner); derivation keeps legacy's exact-name Bleeding matcher (deliberate split from `isCondition` until R7 revisits). | R4a landed: the actor document (derivation dual-run, incl. the creature suite T8 owed), the api/shim/settings/init modules, divergences R4a-1…R4a-6. Decisions: `macroTarget` → client scope; `chooseCover` writes its string directly (widening `MutableDocument.update` to strings would weaken the address guarantee); `compare.js` + DataModels + sheets imported as-is by `init.ts` (its job); deprecation = warn once per name, debug-gated per call. Still open for R4b: fill `CONDITION_IDS` + collapse the two Bleeding readers onto it; android panic-19 variant as content data; `ROLLTABLE_KEYS` from `tableSettings()`; the `fixme` e2e specs; `fireCard`'s fate. For R5/R6: `system.json` `secondaryTokenAttribute: "other.stress"` vs the hook's `hits` bars — decide one owner; CLAUDE.md's stale "273 vitest" line. Owes: fill the `apply` verb with condition machine identity (slug→id, minted like `TABLES`); `severityOf`/`@Gain[health -bleeding]` name-matching dies with it; decide `fireCard`'s fate (no consumer — the dialogs took its role); give `chooseCover`'s answer its write (cover is a string; `MutableDocument.update` is number-typed); decide `macroTarget` scope (client vs world+reword — R3-3 deferred it); carry the android panic-19 variant as content data; regenerate `ROLLTABLE_KEYS` from `tableSettings()`. Embedding the class item on generated characters moves to R7 (it edits the generator); `isRobotic`'s fallback survives until then. |
| R5 — the swap | Opus | done | `R5: the swap…` | **The remade system is live.** 700 vitest · **124/124 e2e** against real Foundry, twice (executor + independent verifier). Legacy deleted (−3,862 lines), recoverable at ancestor `78841ed` (the archive refs were trimmed post-completion, owner decision). Sheets: call-site swaps only; `applyItemRef`/`reloadWeapon` re-grown as thin document methods. All nine hotbar macros one-liners. e2e found and fixed a pre-existing harness bug (`thawDice` deleted Foundry's `randomUniform`). Divergences R5-1 and R4b-1's closure recorded. **R6 inherits:** audit `[done]` tag sweep; CLAUDE.md stale counts (700 vitest / 124 e2e) and stale roll-pipeline prose; `test:e2e` gains the `npm run setup` step (C5); grep `Mosh.macro.wound.*` keys for death (R3-2 removed their reason). **R7 inherits:** `ui/actor/items.js` RANK_BONUS shadow; generator per-item card noise; `Amount` type moves from `documents/actor.ts` to `mutation/`; `chooseWound` default key; embed the class item so `isRobotic`'s fallback dies. |
| R6 — record and close | Sonnet | done | `R6: the record…` | audit sweep 47 `[done]` / 2 `[partial]` (U10, F13) / 2 `[deferred]` (U5, U14) / 26 open, every tag tree-verified; CLAUDE.md + skill refreshed; `test:e2e` runs its own `npm run setup` (C5 closed); `test/content-freshness.test.ts` byte-pins `packs/_source` to the build (C4 closed); 30 dead `Mosh.macro.wound.*` keys removed per lang file; psg-core.md's S9 row updated (orchestrator). U4 found already resolved at R5 — removed from R7's scope. Pre-existing staleness flagged, not fixed: `references/testing.md`, `tsconfig.json`'s one comment. 701 vitest · 124/124 e2e with the new script proven. |
| R7 — sheets adopt services | Opus | done | `R7: the sheets ask…` | 733 vitest · 124/124 e2e (twice) · 19 mutants, 0 survivors. All Owes closed: rules on the documents (`swarmDamage`/`setSwarm`/`stepXp`, `XP_PIPS` drives clamp and track, `RANK_BONUS` shadow dead); five dialogs Svelte, `esc` deleted; one i18n seam (`module/ui/i18n.js` absorbed); class item embedded, android fallback narrowed to the no-class-item case with its retiring migration named; `Amount` home in `mutation/`; silent grants; real remount guard. Divergences R7-1/R7-2. S9 inherits the `value`-rendering gap (recorded in psg-core §S9). **THE LEDGER IS COMPLETE.** | Owes: rules out of components (U5 — swarm math, XP clamp via `XP_PIPS`, the `RANK_BONUS` shadow in `ui/actor/items.js`); the remaining dialog-by-string sites (U10 partial — `generator/dialogs.js`, `promptNewSkill`, `stat-option.js`); reconcile `module/i18n.ts` with `module/ui/i18n.js`; embed the class item on generated characters so `isRobotic`'s fallback dies (F13's last path); move `Amount` from `documents/actor.ts` to `mutation/`; generator per-item card noise; `chooseWound` default key; the dialog mount-once remount caveat. U4 already resolved at R5 — out of scope. |
...
- The ledger below is the running record; git history is the permanent one.
