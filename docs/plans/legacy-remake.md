...
| R4b-1 | Hotbar macros embedded 40–120-line bespoke dialogs; conditions and the Panic table linked macro documents | Five hotbar macros are one-line API calls whose prompts are the system's dialogs; conditions and Panic rows carry `@Check`/`@Gain`/`@Apply` actions; four macros keep a minimal chooser until R5 grows the amount and wound-table prompts | pipeline guards + `test/content-pipeline.test.ts` |
