...
`CircleStat` (deleted in DS8 — no consumer, audit U20), `PipTrack`, `MainStat`, `Field`,
`Tabs`. live-tokens ships Button, Card, Dialog,
...
| DS5 — dead CSS dies | Opus | done | | `scripts/audit-css.ts` (three verdict channels: core-emitted allowlist, dynamic-construction matcher, loose-mention filter; `--assert-none` for guard use; acceptance mutation-proven on `sbt-profile`). Then the deletion: **2043 → 1375 lines** — 85 rules + 1 `@keyframes`, 4 selector trims, the `.window-app` rules, the invalid `color: none !important` declaration (ceiling ratcheted 6→5). Two §1.3/§7 corrections: **`sbt-crew-name` is dead** (only `sbt-profile` lives in SheetHeader) and `grid-6col`–`grid-12col` are unreachable (the only `columns` values in `ROLLTABLE_GROUPS` are 4 and 5) — both deleted on that proof. Kept: `p{margin}` and `nav.sheet-tabs` (DS7's scoping), `sbt-profile` (DS8 renames), `textvaluewrapper-input` (alive via `Field.svelte`'s derived string). Gate: check · vitest · build · **full e2e 142/142 with all 17 baselines byte-exact**. |
...
| DS7 — layer and scope the sheet | Opus | done | | `css/mosh.css` → **`css/mothership.css`**, everything but the 12 `@font-face` wrapped in one `@layer system` block, **17 selector groups scoped** (the §1.2 survivors + `.noborder` and `.condition-modifiers` the mechanical enumeration found). Judgment calls held: `.dice-tooltip` scoped plainly (our cards build their own tooltip markup inside the scope root; the crit-colour leak onto Foundry's own rolls dies deliberately), `.sheet nav.sheet-tabs` → `.mothership.sheet` (co-occurrence verified). **One hazard found by the executor's specificity-flip analysis, fixed by the orchestrator: the `.list-roll`/`.rollable` hover group takes `:where(.mothership)`** — a plain prefix outweighed later `.mosh` colour rules and flipped four hover labels white→black on dark ground, invisible to static baselines. Layered guard un-skipped onto `mothership.css`; `system-loads.spec.ts`'s walker learned to descend into the `@layer` grouping rule. Gate: check · 757 vitest · build · full e2e (141 + the walker fix, then 20/20 re-run) · **all 17 baselines byte-exact**. **Flag for DS8, decide before its first component: Svelte emits component `<style>` CSS unlayered into the bundle** (`.editor-host` is the existing case) — each dissolved component would re-create §1.1 one block at a time; options: layer-wrap inside each block, a build transform, or explicit acceptance; extend the guard to `dist/`. |
| DS8 — dissolution, per-component series | Opus per unit | done — 13 units, roster exhausted | | After DS6, DS7. One component (or coherent group) per commit: styles into scoped `<style>`, reading its own Layer-2 tokens; its `ui-parts` pin retired in the same diff (rule 4); fork-era class names it owns renamed (`sbt-*` in `SheetHeader`). Series-wide decisions (settled at the first unit, Mark 2026-08-15): **full-slot coverage** (colour, font, spacing, radius, border-width all get component slots); **`@layer system` inside every component `<style>` block** (Svelte emits unlayered otherwise — the dist guard in `css-guards.test.ts` enforces zero unlayered rules in the bundle); **component tokens declared on the component's root element**, never `:root`; unmapped literals stay literal with a comment, swept later in one reviewed pass. — **Tabs (first unit):** `--tabs-bar-*`/`--tabs-item-*`, 6 rules moved out of `mothership.css` (1375→1321), 3 chevron rules deleted as provably dead (`.tabs .item` matches no markup — the class-level audit cannot see structural deadness), the `Tabs` class pin retired, `Editor.svelte` layer-wrapped (closes the DS7 flag; dist has zero unlayered rules). `nav.tabs` layout is a contract with core's `blocks.base` now. CI note: `ci.yml` tests before build, so the dist guard always skips there — arms locally only. Gate: check · 758 vitest · build · full e2e 142/142, 17 baselines byte-exact. — **Stat-display group (second unit):** the per-component model met its limit — `.mainstat*`/`.circle-input`/`.circle-statwrapper*` are a **shared stat vocabulary** hand-written across six components, now a comment-declared tier in `mothership.css` (in place, not grouped: two proven order-dependencies — `.transparentBackground` and the `.mainstattext`-vs-hover tie — forbid reordering); call-site conversion is **S9's first work item** (resume points recorded in the unit report: `Generator.svelte:216-266`, `CreatureSheet.svelte:108`, `RollableStat`, 7 raw wrapper sites). **`CircleStat.svelte` deleted** (Mark's call; orphan since the ships cut — closes audit U20; git recovers it for the SBT tier); its 3 rules and 3 specs died with it, audit arbiter exit 0. Known guard blind spot recorded: an unimported component's `<style>` never reaches dist, so the dist guard cannot check it. Gate: check · 755 vitest · build · full e2e 142/142 byte-exact. — **ItemList family (third unit):** `itemlist`/`itemrow`/`itemcell`/`itemcontrols` own their styles (ownership proven class-by-class first); `.skill-name` joined the shared tier (four sheets hand-write it inside lists); `.item-controls` kept its `.items-list` ancestor via `:global()` so the creature sheet's unlisted mount stays untouched; `ItemImage` flagged as the next clean single-component unit; row/list metrics (7/35/24/3/86px) stay literal — off-scale, reserved for the literal sweep. `mothership.css` 1333→1305. Gate found the **system-loads `.mosh`-count spec at its boundary (exactly 100 > 100 fails)** — the count drains by design, so the spec now counts both scope classes and asserts liveness (>30), not size. One unreproduced single-run itemRoll baseline failure observed (passed isolated, in-file, and on two later full runs; no artifact survived) — recorded here so a recurrence is treated as real, not first-time. Gate: check · 754 vitest · build · full e2e 142/142. — **Small primitives (fourth unit):** `ItemImage` and `Field` own their styles; **`CheckField` owns nothing** — `CreatureSettings.svelte:31-40` hand-writes its identical toggle (S9 conversion item, joins the Generator/CreatureSheet stat blocks and the raw wrapper sites). Field's move retired the audit's standing `loose textvaluewrapper-input` entry (rules now scoped beside the derived string that builds the class); the dead `background: 0` reset and its masking inline style collapsed into one tokenized fill. `.item` no longer appears in any mothership.css selector. mothership.css 1305→1270; audit 124 classes 0 dead. Off-scale literals (5px, 2px, 1.5em/0.5em) stay for the reviewed sweep; `--color-black`/`--color-neutral-900` read as primitives where no light-ground border tier exists (the parked DS6b inversion call, Tabs precedent). Gate: check · 754 vitest · build · full e2e 142/142. — **SheetHeader (fifth unit): `sbt` is extinct in code** — `sbt-profile` → `sheet-header-profile` (`.mosh .profile` rejected: different values, two live emitters); one owned rule moved, everything else the markup wears proven shared (Generator/CharacterSheet/CreatureSettings hand-write it — all now S9 call-site material); no pin existed to retire, the first unit where rule 4 had nothing to discharge. `scripts/audit-css.ts`'s comment keeps naming `sbt-profile` as the false positive that justified it — true as history. Structural flag recorded: **the collision guard reads only `css/*.css`, so component-token names (`--tabs-*`, `--field-*`, …) go unchecked against Foundry** — extend it to Svelte styles or dist in a later unit. mothership.css 1270→1263. Gate: check · 754 vitest · build · full e2e 142/142. — **Primitives II (sixth unit):** PipTrack owns and moved (`.circle`/`.circle-f`/`.skill_training_text`; new additive `milestone` state class replaces two inline colour styles; pins adjusted, e2e locators intact); **MinMaxField and RollableStat own nothing** — `minmaxwrapper`/`slant`/`healthmaxtext` hand-written by ArmorBlock and Cover (S9 list grows), `ability-mod`/`stat-roll` are pure DOM hooks with no rule anywhere. Three §4.7-mapped-but-shifting pip colours stay literal for the sweep (the filled-pip `rgb(100,100,100)`→neutral-600 snap is the sweep's most visible pending item). mothership.css 1263→1247, audit 120 classes 0 dead. Gate: check · 754 vitest · build · full e2e 142/142. — **Sections (seventh unit): all three own nothing** — ArmorBlock/HealthBlock/ItemPanel wear only shared vocabulary (`whiteText`/`highlightText` newly proven shared — Cover hand-writes both); HealthBlock and ItemPanel emit zero class names of their own. Found: **four byte-identical inline-style duplications between ArmorBlock and Cover** (`minmaxwrapper` black bar, `slant` white edge, `highlightText` size, `maxhealth-input` flex) — must move as one unit with Cover, queued with S9's call-site work. Comment-only diff; **gate satisfied by bundle byte-diff against HEAD** (js identical, css differs by two comment lines — stronger than a screenshot; e2e deliberately not re-run for a provably render-identical change). The section layer is thin by design; the remaining mass belongs to the sheets. — **CharacterSheet (eighth unit):** 8 exclusively-owned rules moved onto 34 `--charactersheet-*` slots (hand-checked vs foundry2.css: 0 collisions); `savetext` proven owned but **unmovable** (RollableStat renders the element — the cross-boundary class-prop case) and its hover-tie stays in the stylesheet's ordering contract; the `.health` grid split across files (Cover's captions inertly receive its grid-area — commented both sides). Two structurally-dead finds ready for deletion: `.mosh .stress` (no emitter; survives the audit via a loose mention) and the `h1.charname` pair (no `<h1>` exists anywhere). `#111` surface took `--surface-neutral-lowest`; `#111` border took the raw ramp (the parked inversion). mothership.css 1249→1202, audit 112 classes 0 dead. Gate: check · 754 vitest · build · full e2e 142/142. — **CreatureSheet (ninth unit):** 13 owned rules onto 37 `--creaturesheet-*` slots (0 foundry collisions), incl. `creature-mainstat` (claimed off the tier framing — sole writer, own element, never tagged) and the `.creaturedescription` Editor descendants via `:global()`; `creaturestat`/`darkgrey` proven owned-but-unmovable (class props to RollableStat/ItemControl — the `savetext` precedent); both DS7 hover survivals verified intact in the bundle. Structural note repeated with numbers: **the audit's inventory narrows as the series proceeds** (112→100 classes) — moved names leave `css/*.css` and its sight; its "0 dead" verdict covers a shrinking surface. mothership.css 1202→1128, audit 100 classes 0 dead. Gate: check · 754 vitest · build · full e2e 142/142. — **Item-window family (tenth unit): owns almost nothing** — one rule moved (Condition's `condition-modifiers` list); `item-armor-grid` newly tagged shared (Weapon+Armor twin roots); a **fifth inline-style duplication** found for the S9 list (ClassSheet/CharacterSheet trauma-response blocks, one property apart); `header-fields` proven inert on the class sheet (needs a `char-header` ancestor only CharacterSheet/Generator provide — deletion candidate). Executor's whole-bundle collision check: 539 defined custom properties vs Foundry's 398, intersection empty. **Orchestrator widened the baseline set to 20** — `window-item-sheet-armor`/`-ability`/`-condition` added (the moved rule renders only on the condition sheet, which had no baseline; two `-u` runs byte-stable, images reviewed). mothership.css 1128→1131 (comment-heavy, one rule out), audit 99 classes 0 dead. Gate: check · 754 vitest · build · **full e2e 145/145**. — **Generator (eleventh unit):** 9 rules onto 7 `--generator-*` slots + SkillPicker's one owned declaration (`center`, accepted on mechanical proof — no popup baseline exists and the rule is a single uncontested `text-align`); the directory holds RollBox and StatChoice, not the brief's phantom OptionDraft; `clicable-item` claimed (RollBox's one mount is inside the form); `characterGeneratorSkillPopup`/`skillpopupDescription` stay two-writer shared per case law, "unify the two dialog bodies" queued for S9; `dropdownSkill` found rule-less and locator-less — deletion candidate; every retained `.actor-generator` ancestor proven load-bearing (five ties beaten by specificity, not order). mothership.css 1131→1112, audit 90 classes 0 dead. Gate: check · 754 vitest · build · full e2e 145/145. — **Settings windows (twelfth unit):** RolltableConfig moved 4 owned rules onto 18 slots (`circlestattitle` the naming trap correctly claimed; `greyline` reunited with the pattern its `whiteline` twin set; the `button-white` hover survival re-proven in the bundle); **CreatureSettings owns nothing** (CheckField vocabulary, S9 material). `grey` border took `--border-neutral` — the one tier that is byte-exact on a light ground, escaping the parked inversion. No renames (`clicable-item` precedent: misnamed ≠ extinct). Both baselines byte-exact. mothership.css 1112→1090, audit 86 classes 0 dead. Gate: check · 754 vitest · build · full e2e 145/145. — **Guard hardening (Fable direct, `43dbb7d`):** the collision guard unions the built bundle's custom properties, closing the five-unit component-token blind spot; mutation-checked both ways. — **Dialogs (thirteenth unit): the family owns nothing** — its vocabulary is DialogV2 chrome, 5-to-9-writer shared classes, or the dialog radio styling §4.1 recorded (its "until dialogs are restyled" note now closes as "shared, stays"). Four rules deleted on exhaustive-extraction proof: empty `macro_img`/`macro_prompt`, redundant dialog `grid-1col`/`grid-3col` copies; the dialog `grid-2col` copy proven **load-bearing** (it wins the `widegap` tie for `BonusOption` — a probable bug either way, parked: wants a popup baseline before anyone flips it). ArmorBlock/Cover inline duplication: **deliberately left inline** — a shared-tier home would mint a class S9's component collapse deletes. `mosh-dialog-root` proven rule-less (DS9's rename is code+tests only). Deletion-candidate list now: `dropdownSkill`, `header-fields`-on-class-sheet, `macro_img`, `.mosh .stress`, the `h1.charname` pair. mothership.css steady at 1090 (22 rule lines → 22 comment lines), audit 84 classes 0 dead. Gate: check · 754 vitest · build · full e2e 145/145. |
| DS9 — retire `mosh`, sweep the prose | Sonnet + orchestrator | done | | 31 files: 62 selectors `.mosh`→`.mothership` in place (match sets identical — both classes on every root since DS4), `mosh` dropped from all six `classes:` arrays, six chat templates, three Svelte roots; `mosh-sheet-root`→`mothership-sheet-root`, `mosh-dialog-root`→`mothership-dialog-root`, the chat enricher's `data-mosh-action`→`data-mothership-action`, `mosh-generator-` window id, Cover's three inert stray classes, three test temp-dir prefixes; `mosh-dev-entry`→`mothership-dev-entry` (orchestrator). **Justified survivors, each recorded:** `data-mosh-voice` baked into `packs/_source/rolltables/Panic_Check.json` (needs a content regeneration — open item), the local `TEST_WORLD` Foundry world folder named `mosh` (external to the repo; `global-setup.ts`, `setup-test-env.ts`, e2e README), and history-describing comments/tests (deleted `mosh.js`, the pre-rename package id). Prose: CLAUDE.md's naming/CSS/primitives sections rewritten to describe what exists; the skill was already clean. Gate: check · 754 vitest · build · **full e2e 145/145, all 20 baselines byte-exact with the class gone from every rendered element**. |

**Every ledger unit is done (2026-08-15, one session).** Open follow-ups the rows record:
the **reviewed literal sweep** (every off-scale/shifting literal carries a sweep comment at
its site — a deliberate visual-change pass, Mark's call when); the **S9 Svelte audit's
call-site list** (Generator/CreatureSheet stat blocks, CheckField's CreatureSettings twin,
the raw wrapper sites, ArmorBlock/Cover's five inline duplications, the two popup bodies);
the **deletion candidates** (`dropdownSkill`, `header-fields` on the class sheet,
