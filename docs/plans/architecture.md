...
| ~~`weapon.ranges.value`~~ — **inverted, §31**: the *trio* was the dead half | done | `system.range`, a `StringField` enumerating the book's four bands. It was the only range the system had: 22/22 weapons carry a band, `short`/`medium`/`long` are `0` on every one |
