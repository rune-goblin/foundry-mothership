// Rule 11's three standing tests, and the proof that each fails when it should.
//
// content/local/ is empty until C2, so these run against test/fixtures/content — three packs, one
// per document type, cross-linked by @UUID the way the shipped content is. A green run here means
// the machinery works; C2 points the same checks at real records by adding to scripts/content/packs.ts.
...
import { FIXTURE_IDS_BEFORE, FIXTURE_PACKS, FIXTURE_ROOT } from './fixtures/content/packs.ts';

const ROOT = join(import.meta.dirname, '..');
const FIXTURE_REGISTRY = join(FIXTURE_ROOT, 'ids.json');
const COMPENDIA = new Set(FIXTURE_PACKS.map((p) => p.compendium));
...
function run(packs: PackDefinition[] = FIXTURE_PACKS, options: { allocate?: boolean; registryPath?: string } = {}): BuildResult {
  return build({
    root: ROOT,
    packs,
    ...
    tiers: [{ schema: join(FIXTURE_ROOT, 'schema'), data: join(FIXTURE_ROOT, 'local') }],
  });
}

/** The same fixture, with one record rewritten — how each negative case is staged. */
function mutated(pack: string, edit: (record: ContentRecord) => ContentRecord | null): PackDefinition[] {
  return FIXTURE_PACKS.map((def) =>
    def.pack === pack
...
// Catches: a document silently disappearing from the build. Its _id is what every @UUID pointing
// at it resolves through, so a vanished id is 269 potential dead links in the shipped content.
describe('id preservation', () => {
  it('accounts for every id the pre-pipeline packs held', () => {
          ...
          r.contentId === 'flux-capacitor'
            ? {
                ...
                body: {
                  ...
                  system: {
                    description:
                      '<p>@UUID[Compendium.mosh.fixture_macros_1e.Macro.FiXbbbbbbbbbbb01]{Old}</p>',
                  },
                },
              }
            : r,
...
describe('the emitted document', () => {
  ...
  it('keeps modifiers and grantsCondition — they are gameplay data, not metadata', () => {
    const item = built.emitted.find((d) => d.contentId === 'flux-capacitor')!;
    expect((item.document.system as { modifiers: string[] }).modifiers).toEqual(['disadvantage']);
  });

  it('refuses a record whose shape does not match its pack', () => {
    const ids = new IdRegistry(loadRegistry(FIXTURE_REGISTRY));
    const def = FIXTURE_PACKS.find((p) => p.pack === 'gadgets')!;
    const record = def.load(ROOT)[0]!;
    expect(() =>
      emit(def, { ...record, body: { kind: 'Macro', type: 'script', scope: 'global', command: '' } }, ids, stamp),
...
describe('the registry gate', () => {
  ...
  it('does not touch the registry when nothing is allocated', () => {
    ...
    run(FIXTURE_PACKS, { allocate: true, registryPath });
...
describe('the build manifest', () => {
  it('records provenance outside the packs, never inside one', () => {
    ...
    const manifest = JSON.parse(readFileSync(path, 'utf8')) as {
      documents: { id: string; provenance: { tier: string; source: string } }[];
    };
    expect(manifest.documents.length).toBe(result.emitted.length);
    expect(manifest.documents.every((d) => d.provenance.tier === 'local')).toBe(true);

    const packed = [...result.files.values()].join('');
    expect(packed).not.toContain('provenance');
    expect(packed).not.toContain('fixtures/content/local');
  });
});
...
describe('validation', () => {
  it('rejects a record the schema does not allow', () => {
    ...
    copyFileSync(join(FIXTURE_ROOT, 'local/quips.json'), join(broken, 'quips.json'));
    copyFileSync(join(FIXTURE_ROOT, 'local/mishaps.json'), join(broken, 'mishaps.json'));
    const gadgets = JSON.parse(readFileSync(join(FIXTURE_ROOT, 'local/gadgets.json'), 'utf8')) as {
      severity: number;
    }[];
    gadgets[0]!.severity = -1;
    writeFileSync(join(broken, 'gadgets.json'), JSON.stringify(gadgets));
    expect(() =>
      build({
        root: ROOT,
        packs: FIXTURE_PACKS,
        registryPath: FIXTURE_REGISTRY,
        outDir: temp('out'),
        tiers: [{ schema: join(FIXTURE_ROOT, 'schema'), data: broken }],
      }),
